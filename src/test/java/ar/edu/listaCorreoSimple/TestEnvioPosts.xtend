...
class TestEnvioPosts {
	...
	def void alumnoNoPuedeEnviarPostAListaProfes() {
		listaProfes.enviar(mensajeAlumnoRecursividad)
	}

	@Test
	def void alumnoPuedeEnviarMailAListaAbiertaAunSinPertenecerALaLista() {
		Assert.assertEquals(0, stubMailSender.mailsDe("alumno@uni.edu.ar").size)
		listaAlumnos.enviar(mensajeAlumnoRecursividad)
	...
	def void alumnoEnvia2MailsYSeRegistranAmbosMailsEnElStub() {
		Assert.assertEquals(0, stubMailSender.mailsDe("alumno@uni.edu.ar").size)
		listaAlumnos.enviar(mensajeAlumnoRecursividad)
		listaAlumnos.enviar(mensajeAlumnoOrdenSuperior)
	...
	def void alumnoQuiereEnviarDosMailsPeroElSegundoFallaYSoloSeEnviaUno() {
		...
		try {
			listaAlumnos => [
				...
				enviar(mensajeAlumnoRecursividad)
				enviar(mensajeAlumnoOrdenSuperior)
			]
	...
	def void alumnoEnvia2MailsConMockitoQueGeneraUnaCascaraVaciaDeLaInterfazQueHacePrintln() {
		...
		listaAlumnos => [
			...
			enviar(mensajeAlumnoRecursividadOk)
			enviar(mensajeAlumnoOrdenSuperior)
		]
